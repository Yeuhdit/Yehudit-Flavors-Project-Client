...
// const steps = ['שם וזמן', 'קטגוריות וקושי', 'מרכיבים', 'שלבי הכנה', 'תמונה ופרטיות'];
...
//     ingredients: [''], instructions: [''], isPrivate: false, image: null, youtubeUrl: '' // 🔥 הוספנו youtubeUrl
//   });
...
//           image: null,
//           youtubeUrl: recipeToEdit.youtubeUrl || '' // 🔥 טעינת יוטיוב בעריכה
//         });
...
//         setFormData({
//           name: '', preparationTime: '', difficulty: '', categories: [], levels: [],
//           ingredients: [''], instructions: [''], isPrivate: false, image: null, youtubeUrl: ''
//         });
...
//       data.append('youtubeUrl', formData.youtubeUrl); // 🔥 הוספת היוטיוב לשליחה
//     }
...
//               renderInput={(params) => (
//                 <TextField {...params} label="קטגוריות (אופציונלי)" {...modernInputProps} sx={{ mb: 4, ...modernInputProps.sx }} />
//               )}
//               renderTags={(value, getTagProps) =>
//                 value.map((option, index) => (
//                   <Chip label={typeof option === 'string' ? option : (option.description || option.name)} {...getTagProps({ index })} className="designer-chip" />
//                 ))
//               }
//             />
...
//                   <input type="text" className="clean-input" placeholder="לדוגמא: 2 כוסות קמח מנופה..."
//                     value={ingredient} onChange={(e) => handleArrayChange(index, 'ingredients', e.target.value)}
//                   />
...
//                   <textarea className="clean-textarea" placeholder="תארי את שלב ההכנה בצורה ברורה..."
//                     value={instruction} onChange={(e) => handleArrayChange(index, 'instructions', e.target.value)} rows={3}
//                   />
...
//         return (
//           <Box className="step-content-inner fade-in">
//             <Typography variant="h2" className="step-title">תמונה, וידאו וסיום</Typography>
...
//                   <IconButton type="button" className="remove-image-btn" onClick={handleRemoveImage}>
//                     <CloseRoundedIcon />
//                   </IconButton>
//                 </div>
//               )}
...
//             {/* 🔥 שדה קישור ליוטיוב שביקשת */}
//             <TextField 
//               label="קישור לסרטון הדרכה (YouTube) - אופציונלי" 
//               name="youtubeUrl" 
//               fullWidth 
//               value={formData.youtubeUrl} 
//               onChange={handleTextChange} 
//               {...modernInputProps} sx={{ mb: 4, mt: 4, ...modernInputProps.sx }}
//             />
            
//             <FormControlLabel 
//               control={<Checkbox checked={formData.isPrivate} onChange={handleCheckboxChange} sx={{ color: '#ccc', '&.Mui-checked': { color: '#ff7e5f' } }} />} 
//               label={<Typography sx={{ fontSize: '1rem', color: '#666', fontWeight: 500 }}>שמור במחברת הפרטית שלי (לא יפורסם בקהילה)</Typography>} 
...
//         {(serverMsg || error) && (
//           <Alert severity={(isError || error) ? "error" : "success"} className="modern-alert fade-in">{error || serverMsg}</Alert>
//         )}
...
//                 <Button type="button" onClick={handleSubmit} variant="contained" disabled={loading} sx={{ backgroundColor: '#27ae60', borderRadius: '12px', fontWeight: 'bold' }} endIcon={<SaveRoundedIcon />}>
//                   {loading ? <CircularProgress size={20} color="inherit" /> : 'שמור וצא'}
//                 </Button>
//               )}
//               {activeStep === steps.length - 1 ? (
//                 <Button type="button" onClick={handleSubmit} variant="contained" disabled={loading} className="master-submit-btn" endIcon={<CheckCircleRoundedIcon />}>
//                   {loading ? <CircularProgress size={24} color="inherit" /> : (isEditMode ? 'שמירת שינויים!' : 'פרסום המתכון!')}
//                 </Button>
//               ) : (
...
function AddRecipe() {
  ...
  const handleSubmit = async (e) => {
    ...
    // 🔥 התיקון: בדיקה שגם קטגוריה נבחרה!
        ...
        return (
          <Box className="step-content-inner fade-in">
            ...
            <Autocomplete
              ...
              onChange={handleCategoriesChange}
              renderInput={(params) => (
                // 🔥 התיקון: שונה מ-"אופציונלי" ל-"חובה"
                <TextField {...params} label="קטגוריות *" {...modernInputProps} sx={{ mb: 4, ...modernInputProps.sx }} />
              )}
              renderTags={(value, getTagProps) =>
                value.map((option, index) => (
                  <Chip key={index} label={typeof option === 'string' ? option : (option.description || option.name)} {...getTagProps({ index })} className="designer-chip" />
                ))
              }
            />
  ...
  return (
    <div className="layout-2028-wrapper stepped-form-wrapper" dir="rtl" ref={formRef}>
      ...
      {/* הזיקוקים מופעלים כאן */}
      {showConfetti && <Confetti width={width} height={height} recycle={false} numberOfPieces={500} gravity={0.2} zIndex={99999} />}
