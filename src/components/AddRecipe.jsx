...
const steps = ['שם וזמן', 'קטגוריות וקושי', 'מרכיבים', 'שלבי ההכנה', 'מדיה ופרטיות'];
    ...
    if (isEditMode && recipesState.recipes?.length > 0) {
      ...
      if (recipeToEdit) {
        setFormData({
          ...
          level: recipeToEdit.level?._id || recipeToEdit.level || '',
          ...
          youtubeUrl: recipeToEdit.youtubeUrl || '',
          image: null
        });
        if (recipeToEdit.imageUrl) {
          const url = recipeToEdit.imageUrl;
          ...
          const fullImageUrl = BASE_URL + '/images/' + cleanName;
        ...
        return (
          <Box className="step-content-inner fade-in">
            ...
            <FormControl fullWidth {...modernInputProps}>
              <InputLabel>רמת התאמה (אופציונלי)</InputLabel>
              <Select 
                ref={selectRef}
                name="level" 
                value={formData.level} 
                onChange={handleLevelChange}
                label="רמת התאמה (אופציונלי)"
              >
                <MenuItem value="">בחרי רמה</MenuItem>
                {allLevels.map((level) => (
